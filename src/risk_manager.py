"""
risk_manager.py — Position sizing and risk rule enforcement.
...
class RiskManager:
    ...
    def can_open_trade(self) -> tuple[bool, str]:
        ...
        if self.drawdown_pct >= config.MAX_DRAWDOWN_HALT_PCT:
            return False, (
                f"Drawdown {self.drawdown_pct:.1f}% ≥ halt threshold "
                f"{config.MAX_DRAWDOWN_HALT_PCT}%"
            )
    ...
    def check_exit(
        ...
        if direction == "long":
            if pnl_pct <= -config.BUY_STOP_LOSS_PCT:
                return True, "sl"
            if pnl_pct >= config.BUY_TARGET_PCT:
                return True, "target"
        else:  # short
            if pnl_pct <= -config.SELL_STOP_LOSS_PCT:
                return True, "sl"
            if pnl_pct >= config.SELL_TARGET_PCT:
                return True, "target"
